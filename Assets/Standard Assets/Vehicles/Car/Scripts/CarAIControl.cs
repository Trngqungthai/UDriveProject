        ...
        private void OnCollisionStay(Collision col)
        {
            ...
            if (col.rigidbody != null)
            {
                ...
                if (otherAI != null)
                {
                    ...
                    // but who's in front?...
                    if (otherAI != null && Vector3.Angle(transform.forward, otherAI.transform.position - transform.position) < 90)
                    {
                        // the other ai is in front, so it is only good manners that we ought to brake...
                        m_AvoidOtherCarSlowdown = 0.5f;
                    }
                    if (PlayerCar != null && Vector3.Angle(transform.forward, PlayerCar.transform.position - transform.position) < 90)
                    {
                        // the other ai is in front, so it is only good manners that we ought to brake...
                        m_AvoidOtherCarSlowdown = 0.5f;
                    }
